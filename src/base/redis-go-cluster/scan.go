...
func convertAssignError(d reflect.Value, s redisError) (err error) {
...
func GetStructFields(dest interface{}) (fields [][]byte, err error) {
	d := reflect.ValueOf(dest)
	if d.Kind() != reflect.Ptr || d.IsNil() {
		return nil, errScanStructValue
	}
	...
	for _, field := range ss.m {
		fields = append(fields, []byte(field.name))
	}
	return
}

func ScanStructByValue(src []interface{}, names [][]byte, dest interface{}) error {
	if len(src) != len(names) {
		return errors.New("redis: names and valus is not match")
	}
	d := reflect.ValueOf(dest)
	if d.Kind() != reflect.Ptr || d.IsNil() {
		return errScanStructValue
	...
	for i := 0; i < len(src); i++ {
		...
		name := names[i]
		if name == nil {
			return errors.New("redis: ScanStuct key not a string value")
		}
		fs := ss.fieldSpec(name)
		if fs == nil {
			continue
